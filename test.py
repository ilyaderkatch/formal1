...
def test_basic():
    ...
    assert main.find_answer(['abc..*', '9', '2']) == 'NO' # (abc)*
    assert main.find_answer(['ab.*cab..*.', '5', '1']) == 'YES' # (ab)*(cde)*
...
def test_error_new_letters():
    assert main.find_answer(['ad+', '3', '2']) == 'ERROR'
    assert main.find_answer(['Ab+*', '3', '2']) == 'ERROR'
